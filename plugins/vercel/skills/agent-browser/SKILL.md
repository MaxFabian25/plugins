...
agent-browser session list            # Inspect active sessions before targeted close
...
agent-browser wait -l networkidle     # Network-idle fallback only when no app-specific cue exists
...
agent-browser session list             # Review sessions before any broader teardown
```
